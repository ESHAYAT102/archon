...
general {
     ...
     gaps_out = 4
     border_size = 1
     col.active_border = rgba(cba6f766) rgba(cba6f728) 45deg
...
windowrule = match:title ^(Library|File Upload|Open File|Save File|Select a Folder|Open Folder|Upload(.*)|Save As|Select File|Select Folder|Choose File|Zen Browser|Sign in - Google Accounts — Zen Browser).*$, float on, size 700 500, center on
windowrule = match:class xdg-desktop-portal-gtk, float on, size 800 600, center on
EOT
...
# Notifications
bindd = SUPER, COMMA, Dismiss last notification, exec, swaync-client -C
bindd = SUPER CTRL, COMMA, Toggle silencing notifications, exec, swaync-client -d -sw
...
bindd = , Print, Screenshot, exec, hyprshot -m output -m eDP-1 -o /tmp
bindd = SUPER, Print, Screenshot, exec, hyprshot -m region -o /tmp --raw | satty --filename -
